...
echo -e "Enter old CONFIG__PROJECT__NAME (e.g. UNSET__CONFIG__PROJECT__NAME):"
...
NEW_PROJECT_NAME=$(grep -E '^CONFIG__PROJECT__NAME=' config/.env | cut -d '=' -f2-)

if [[ -z "$NEW_PROJECT_NAME" ]]; then
    echo "ERROR: CONFIG__PROJECT__NAME not set in config/.env"
    exit 1
fi
